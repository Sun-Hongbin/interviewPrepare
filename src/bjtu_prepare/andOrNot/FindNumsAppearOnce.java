...
public class FindNumsAppearOnce {
	...
	public void findNumsAppearOnce(int[] array, int[] num1, int[] num2) {
		...
		int indexOf1 = findFirst1Position(temp);

		for (int j : array) {
			if (isOneOrZero(j, indexOf1)) {
				num1[0] ^= j;
			} else {
				num2[0] ^= j;
			}
	...
	// 找出这个数2进制形态下右边第一个1的位置
	public int findFirst1Position(int num) {
		int index = 0;
	...
	// 第n位是1分为1组，0分为一组
	public boolean isOneOrZero(int num, int n) {
